...
class _ControllerPageState extends State<ControllerPage> {
  ...
  MyCustomMarkers myMarkers = MyCustomMarkers(size: 5);
  ...
  late StreamSubscription<List<int>> _depthSubscription;
  late StreamSubscription<List<int>> _rotationSubscription;
  ...
  void initState() {
    ...
    _connectionStateSubscription = widget.device.connectionState.listen((state) {
      if (state == BluetoothConnectionState.disconnected) {
        ...
        print("Ble device diconnection: ${widget.device.disconnectReason?.code} ${widget.device.disconnectReason?.description}");
        if(mounted) {
  ...
  Future<void> procesCharacteristics(List<BluetoothCharacteristic> characteristics) async {
    try {
      for (BluetoothCharacteristic c in characteristics) {
        print('RECEIVED CHARACTERISTIC UUID:  ${c.characteristicUuid} (from service: ${c.serviceUuid})');
  ...
  Future<void> _updateOrientation() async {
    ...
    if (rotation != null) {
      _rotationSubscription = rotation!.lastValueStream.listen((bytes) async {
        _bytes = bytes;
        decodedData = String.fromCharCodes(_bytes);
        if (mounted) {
          setState(() {
            if (decodedData != null) {
              droneRotation = double.tryParse(decodedData!);
              print("droneRotation: $droneRotation");
            }
          });
        }
      });
      ...
      try {
        await rotation!.setNotifyValue(true);
      } catch (e) {
        print('error when subscribing: $e');
      }
    }
  ...
  Future<void> updateDronePosition() async {
    if (latitudePos != null && longitudePos != null && droneRotation != null) {
  ...
  Future<void> updateTraveledDistance() async {
    if (latitudePosBefore != null && longitudePosBefore != null) {
       double isdistancereal = calculateDistance(LatLng(latitudePosBefore!, longitudePosBefore!), LatLng(latitudePos!, longitudePos!));
      if (isdistancereal < 1) {
        traveledDistanceDouble += isdistancereal;
      }
  ...
  Future<void> _updateDepthMarkers() async {
    List<int> bytesDepth = [];
    String? decodedDataDepth;
    double? depthDouble;

    if (depth != null) {
      _depthSubscription = depth!.lastValueStream.listen((bytes) async {
        bytesDepth = bytes;
        decodedDataDepth = String.fromCharCodes(bytesDepth);
        depthDouble = double.tryParse(decodedDataDepth!);
        if (mounted) {
          setState(() {
            if (latitudePos != null && longitudePos != null && depth != null) {
              myMarkers.addMarker(
                depth: depthDouble!, 
                position: LatLng(latitudePos!, longitudePos!), 
                zoom: 7, 
                id: "$latitudePos"
              );
            }
      ...
      try {
        await depth!.setNotifyValue(true);
      } catch (e) {
        print('error when subscribing: $e');
      }
    }
      ...
      body: Stack(
        children: <Widget>[
          GoogleMap(
            ...
            //onCameraIdle: _onCameraIdle, // Cambia el tamaño de los marcadores
      ...
      child: ListView(
        children: [
          ...
          ListTile(
            ...
            horizontalTitleGap: 0,
            onTap: () async {
              Dialogs.changeChannelDialog(context, widget.controller, widget.characteristic, value);
            },
          ),
          Padding(
            padding: EdgeInsets.only(left: 30, right: 20),
